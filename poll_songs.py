...
# -*- coding: utf-8 -*-
"""
poll_songs.py — 100% Golden Path + Pipeline (Suno old-docs)
• Läser jobid_aktiv.json (skapad av create_songs.py)
• Pollar: GET https://api.sunoapi.org/api/v1/generate/record-info?taskId=...
• Vid SUCCESS körs produktionssteg:
  1) Ladda ner MP3 ➜ ./out/<taskId>.mp3
  2) Skapa jobbmapp ./jobs/<taskId>__<slug_title>/
  3) Skriv manifest.json (prompt, endpoint, record_info, sunoData, paths)
  4) Snapshot: jobid_aktiv.json och sunoprompt_aktiv.json ➜ jobbmapp
  5) Kopiera/renama MP3 ➜ ./jobs/.../<slug_title>.mp3
  6) Uppdatera jobid_aktiv.json (COMPLETED) och append till jobid_done.json
  7) (valfritt) Hämta imageUrl/coverUrl
"""
...
def main():
    ap = argparse.ArgumentParser(description="poll_songs.py — Golden Path + Pipeline")
    ...
    for jid, item in id_pairs:
        ...
        for _ in range(args.timeout):
            ...
            if st in STATUS_DONE:
                ...
                # 5) copy/rename mp3 into job folder
                try:
                    (job_dir / f"{slug}.mp3").write_bytes(mp3_out.read_bytes())
                ...
                active.setdefault("meta",{})._update = now()
